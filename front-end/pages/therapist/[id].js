...
const Therapist = () => {
  ...
  const { data, error } = useSWR(`${config.apiBaseUrl}/therapist/${id}`);
  ...
  return (
    <Card>
      <Header
        headline={"Explore Therapists"}
        subtext={`Occupation: ${data.occupation ? data.occupation : "N/A"}`}
      />
      ...
      <RegularCard>
        ...
        <div className=" text-left m-2">
          ...
          <ul>
            <p>
              <span className=" font-bold">favExercises: </span>{" "}
              {data.favExercises.length == 0 && "N/A"}{" "}
            </p>
          ...
          <p>
            <span className=" font-bold">Number of Patients: </span>
            {data.patients.length}
          </p>
          {/* current patient can vote if the therapist's patient */}
          {data?.patients?.some(
            (patient) => patient._id === patientId && userType == "patient"
          ) && (
            <Rate
              patientId={patientId}
              therapistId={id}
