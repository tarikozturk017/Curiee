...
const Patients = () => {
  ...
  // const { data: patients, error } = useSWR(`${config.apiBaseUrl}/patient/myPatients?therapistId=${therapistId}`, async (url) => {
    ...
    if (therapist._id) {
      const fetchTherapist = async () => {
        const res = await fetch(
          `${config.apiBaseUrl}/therapist/${therapist._id}`
        );
  ...
  return (
    <>
      {therapistData.patients && (
        <Card>
          <Header
            headline={`My Patients`}
            subtext={`List of your ${
              displayActivePatients ? "active patients" : "previous patients"
            }, please click on "${
              displayActivePatients ? "Previous Patients" : "Active Patients"
            }" 
        button to see your ${
          displayActivePatients ? "previous" : "active"
        } patients`}
          />
          ...
          <button
            onClick={() => setDisplayActivePatients(!displayActivePatients)}
